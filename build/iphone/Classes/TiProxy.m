...
//Rare exceptions to indicate a bug in the _cornerstone code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_cornerstone = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_cornerstone]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.cornerstone.module"
